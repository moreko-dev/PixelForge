...
function SingleLayerProperties({ layer }) {
    ...
    const { projectState, forceUpdateProject, selectionManager } =
    ...
    const removeLayerModalOnSubmit = () => {
        const currentLayerIndex = projectState.findIndex(
            (l) => l.id === layer.id,
        );
        projectState.splice(currentLayerIndex, 1);
        if (layer.id === selectionManager.selectedItem) {
    ...
    return (
        <>
            <div className="properties-section__content--dropdown">
                ...
                >
                    {layer.type === layerType.IMAGE_LAYER ? (
                        <ImageLayerProperties layerID={layer.id} />
                    ) : layer.type === layerType.TEXT_LAYER ? (
                        <TextLayerProperties layerID={layer.id} />
                    ) : layer.type === layerType.PEN_LAYER ? (
                        <BrushLayerProperties layerID={layer.id} />
                    ) : layer.type === layerType.SHAPE_LAYER ? (
